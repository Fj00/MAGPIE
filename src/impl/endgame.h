...
typedef struct EndgameArgs {
  ...
  // NOTE: the former base_thread_index (movegen scratch-slot offset) is gone.
  // get_movegen() now hands each pthread its own MoveGen from a slot pool with
  // an auto-release destructor, so concurrent solves cannot collide.
...
static inline void endgame_args_fill(
    ...
    const double hard_time_limit, const int sign_stable_k,
    const uint64_t seed,
  ...
  endgame_args->sign_stable_k = sign_stable_k;
